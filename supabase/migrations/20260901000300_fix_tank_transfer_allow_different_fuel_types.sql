-- ---------------------------------------------------------------------------
-- 20260901000300 · ALLOW TANK TRANSFERS ACROSS FUEL TYPES
--
-- The fuel-type matching validation is intentionally absent. An internal
-- transfer may move stock between any two distinct, active tanks in a station.
-- Fuel types are retained as source/destination snapshots for audit only.
-- ---------------------------------------------------------------------------

alter table public.tank_transfers
  add column if not exists source_fuel_type_id uuid references public.fuel_types (id) on delete restrict,
  add column if not exists destination_fuel_type_id uuid references public.fuel_types (id) on delete restrict;

-- Preserve the source fuel type for existing transfer records without changing
-- their balances or deleting any history.
update public.tank_transfers
   set source_fuel_type_id = fuel_type_id
 where source_fuel_type_id is null;
...
begin
  ...
  if not exists (
    select 1 from public.profiles
     where id = v_actor_id and station_id = p_station_id and is_active
  ) then
  ...
  if p_request_token is not null then
    select id into v_transfer_id
     ...
     where request_token = p_request_token and station_id = p_station_id
     for update;
  ...
  if not found then
    raise exception 'الخزان المستهدف غير موجود في هذه المحطة.' using errcode = 'foreign_key_violation';
  end if;
  if not v_destination.is_active then
    raise exception 'الخزان المستهدف غير نشط.' using errcode = 'check_violation';
  end if;

  -- Intentionally no comparison of v_source.fuel_type_id and
  -- v_destination.fuel_type_id: different fuel types are permitted.
  select coalesce(quantity, 0) into v_source_balance
    from public.tank_balances where tank_id = p_source_tank_id for update;
  ...
  select coalesce(quantity, 0) into v_destination_balance
    from public.tank_balances where tank_id = p_destination_tank_id for update;
  ...
  insert into public.tank_transfers (
    ...
    note, request_token, created_by, created_at
  ) values (
  ...
  update public.tank_transfers
     set source_txn_id = public.fn_post_transaction(
           p_source_tank_id, 'tank_transfer', -v_quantity,
           coalesce(p_business_date, current_date), null, 'tank_transfers',
           v_transfer_id, coalesce(p_note, 'نقل داخلي بين الخزانات'), v_actor_id
         ),
         destination_txn_id = public.fn_post_transaction(
           p_destination_tank_id, 'tank_transfer', v_quantity,
           coalesce(p_business_date, current_date), null, 'tank_transfers',
           v_transfer_id, coalesce(p_note, 'نقل داخلي بين الخزانات'), v_actor_id
         )
