-- Manager-only transactional correction paths.
-- Operational records remain append-only: the old row is voided and a corrected row is inserted.
...
declare old_sale public.sales%rowtype; session_status public.reconciliation_status; new_id uuid; actor uuid := coalesce(p_actor, auth.uid());
begin
...
returns uuid language plpgsql security definer set search_path = public, pg_temp as $$
declare old_delivery public.deliveries%rowtype; session_status public.reconciliation_status; new_id uuid; actor uuid := coalesce(p_actor, auth.uid());
begin
  if not public.app_is_manager() then raise exception 'هذا الإجراء متاح للمدير فقط.' using errcode = 'insufficient_privilege'; end if;
...
revoke all on function public.fn_manager_replace_sale(uuid,jsonb,text,uuid) from public;
revoke all on function public.fn_manager_replace_delivery(uuid,jsonb,text,uuid) from public;
grant execute on function public.fn_manager_replace_sale(uuid,jsonb,text,uuid) to authenticated;
grant execute on function public.fn_manager_replace_delivery(uuid,jsonb,text,uuid) to authenticated;
