-- Allow the explicit operational reset workflow to remove dependent operational rows.
-- All normal API writes remain append-only; the flag is local to the definer
-- function and is never accepted from a request payload.
...
begin
  ...
  update public.sales
     set status = 'voided', voided_by = p_actor_id, voided_at = now(), void_reason = 'إعادة تهيئة بيانات التشغيل بطلب المدير'
  ...
  select p_station_id, p_actor_id, 'manager', 'delete', 'operational_data_reset', v_code,
         'تم حذف بيانات التشغيل مع إبقاء إعدادات المحطة وسجل التدقيق.';
end;
$$;
