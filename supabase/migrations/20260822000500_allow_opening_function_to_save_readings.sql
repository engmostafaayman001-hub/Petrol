-- The opening RPC is the only path allowed to write initial meter readings.
-- Enable engine mode before its line updates so the immutability guard does
-- not block the initial write.
...
begin
  ...
  select id, status into v_session_id, v_existing_status from public.reconciliation_sessions where station_id = p_station_id and business_date = current_date and shift_id = p_shift_id for update;
  ...
  on conflict (station_id, business_date, shift_id) do nothing returning id into v_session_id;
