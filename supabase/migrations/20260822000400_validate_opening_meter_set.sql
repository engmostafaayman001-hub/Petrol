-- Validate the exact operational meter set before opening a reconciliation.
...
begin
  ...
  select string_agg(pm.code, ', ' order by pm.code)
    into v_missing
    from public.pump_meters pm
    join public.tanks t on t.id = pm.tank_id
  ...
  if jsonb_array_length(p_opening_meters) <> (
    select count(*) from public.pump_meters pm join public.tanks t on t.id = pm.tank_id
     where pm.station_id = p_station_id and pm.is_active and t.is_active and t.status = 'operational'
  ) then raise exception 'عدد قراءات العدادات لا يطابق عدد العدادات التشغيلية.' using errcode = 'check_violation'; end if;
  ...
  if v_session_id is null then
    select id, status into v_session_id, v_existing_status from public.reconciliation_sessions where station_id = p_station_id and business_date = current_date and shift_id = p_shift_id;
    if v_existing_status = 'open' then return v_session_id; end if;
    raise exception 'تم فتح هذه الوردية اليوم وإغلاقها بالفعل. اختر وردية أخرى.' using errcode = 'unique_violation';
  end if;
  ...
  update public.reconciliation_lines l set opening_tank_qty = public.fn_vol(coalesce(b.quantity, 0))
    from public.tank_balances b where l.session_id = v_session_id and b.tank_id = l.tank_id;
