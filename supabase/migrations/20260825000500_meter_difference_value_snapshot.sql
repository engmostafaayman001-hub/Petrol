-- Store the sale price used by each session meter reading. This keeps old
-- sessions stable when fuel prices change later.
alter table public.reconciliation_meter_readings
  add column if not exists unit_price numeric(14,4) not null default 0
    check (unit_price >= 0);
alter table public.reconciliation_meter_readings
  add column if not exists meter_value numeric(16,2) generated always as (
    case when closing_reading is null then null else round((closing_reading - opening_reading) * unit_price, 2) end
  ) stored;

update public.reconciliation_meter_readings r
set unit_price = coalesce(f.selling_price, 0)
from public.reconciliation_lines l
join public.fuel_types f on f.id = l.fuel_type_id
where r.reconciliation_line_id = l.id and r.unit_price = 0;
