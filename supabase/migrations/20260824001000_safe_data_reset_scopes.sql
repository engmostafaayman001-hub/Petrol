-- Safe reset scopes. Both operations require a manager, station-code
-- confirmation, and no open session. Station configuration and user accounts
-- are never removed.
...
begin
  ...
  -- The direct session FK is restrictive. Detach metadata first, then void
  -- the operational rows so their history and inventory reversals remain safe.
  ...
  update public.sales set status = 'voided', voided_by = p_actor_id, voided_at = now(), void_reason = 'إعادة تهيئة بيانات التشغيل بطلب المدير'
   where station_id = p_station_id and status = 'active';
  update public.deliveries set status = 'voided', voided_by = p_actor_id, voided_at = now(), void_reason = 'إعادة تهيئة بيانات التشغيل بطلب المدير'
   where station_id = p_station_id and status = 'active';

  insert into public.audit_logs (station_id, actor_id, actor_role, action, entity, entity_label, reason)
  values (p_station_id, p_actor_id, 'manager', 'delete', 'operational_data_reset', v_code, 'تم مسح بيانات التشغيل مع إبقاء إعدادات المحطة والحسابات.');
end;
$$;
...
begin
  ...
  insert into public.audit_logs (station_id, actor_id, actor_role, action, entity, entity_label, reason)
  values (p_station_id, p_actor_id, 'manager', 'delete', 'session_data_reset', v_code, 'تم مسح الجلسات والبيانات غير الأساسية مع إبقاء العمليات المالية.');
end;
$$;
