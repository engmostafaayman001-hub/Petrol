-- A sale channel is a breakdown of a sale, never an additional inventory move.
alter table public.sales
  add column if not exists sales_channel text not null default 'regular';

alter table public.sales drop constraint if exists sales_sales_channel_check;
alter table public.sales add constraint sales_sales_channel_check
  check (sales_channel in ('regular', 'manual'));
...
begin
  ...
  select count(*) > 0 and count(*) filter (where r.closing_reading is not null) = count(*),
...
grant execute on function public.fn_session_sales_summary(uuid) to authenticated;