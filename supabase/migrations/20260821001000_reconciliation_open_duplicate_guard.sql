-- Prevent duplicate opening attempts for the same station, date and shift.
-- Reopening an already-open session is idempotent; a submitted session stays immutable.
...
declare
  v_session_id uuid;
  v_existing_status public.reconciliation_status;
  v_seq smallint;
...
begin
  ...
  if jsonb_array_length(p_opening_meters) <> (
    select count(*) from public.pump_meters pm join public.tanks t on t.id = pm.tank_id
     where pm.station_id = p_station_id and pm.is_active and t.is_active and t.status = 'operational'
  ) then raise exception 'يجب تسجيل قراءة بداية لكل العدادات التشغيلية.' using errcode = 'check_violation'; end if;

  select id, status into v_session_id, v_existing_status
    from public.reconciliation_sessions
   where station_id = p_station_id and business_date = current_date and shift_id = p_shift_id
   for update;
  if v_session_id is not null then
    if v_existing_status = 'open' then return v_session_id; end if;
    raise exception 'تم فتح هذه الوردية اليوم وإغلاقها بالفعل. اختر وردية أخرى.' using errcode = 'unique_violation';
  end if;
  ...
  on conflict (station_id, business_date, shift_id) do nothing
  ...
  for v_meter in select value from jsonb_array_elements(p_opening_meters) loop
    ...
    if v_value is null or v_value < 0 or not exists (
      select 1 from public.pump_meters pm join public.tanks t on t.id = pm.tank_id
       where pm.id = v_meter_id and pm.station_id = p_station_id and pm.is_active
         and t.is_active and t.status = 'operational'
    ) then raise exception 'توجد قراءة عداد غير صحيحة أو لعداد غير تشغيلي.' using errcode = 'check_violation'; end if;
    select pm.tank_id into v_tank_id from public.pump_meters pm join public.tanks t on t.id = pm.tank_id
     where pm.id = v_meter_id and t.is_active and t.status = 'operational';
    update public.reconciliation_lines set meter_id = v_meter_id, opening_meter = public.fn_vol(v_value)
