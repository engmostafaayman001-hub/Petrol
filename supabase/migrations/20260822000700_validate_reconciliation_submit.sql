-- Validate every reconciliation line before submission, including unlinked meters.
...
begin
  ...
  select string_agg(coalesce(pm.code, 'عداد غير مربوط'), ', ' order by rl.id)
   ...
   where rl.session_id = p_session_id
     and (rl.meter_id is null or rl.opening_meter is null or rl.closing_meter is null);
