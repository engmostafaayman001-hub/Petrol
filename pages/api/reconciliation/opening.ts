...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const actor = await requireStationShiftManager(req, session.station_id);
    ...
    if (error) return res.status(/المدير فقط|صلاحية|permission|insufficient|المشرف/i.test(error.message) ? 403 : 400).json({ error: error.message });
