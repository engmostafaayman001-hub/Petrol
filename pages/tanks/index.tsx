  ...
  return <PageLayout title="الخزانات والمخزون"><main className="inventory-page">
    ...
    {transferOpen && <div className="modal-backdrop" onMouseDown={() => setTransferOpen(false)}><form onMouseDown={(event) => event.stopPropagation()} onSubmit={submitTransfer} className="ui-card form-card modal-card" style={{ maxWidth: 560 }}>
      ...
      <div className="space-y-4">
        ...
        {sourceTank && destinationTank && hasDifferentFuelType && <div className="rounded border border-amber-400 bg-amber-50 p-4 text-sm text-amber-950 shadow-sm"><p className="mb-2 text-base font-bold uppercase tracking-wide text-amber-900">Confirm transfer</p><p className="mb-2 font-semibold">نوع الوقود مختلف بين الخزانين.</p><p className="mb-1">المصدر: <strong>{sourceTank.fuel_name || 'وقود'}</strong></p><p className="mb-1">المستهدف: <strong>{destinationTank.fuel_name || 'وقود'}</strong></p><p className="mb-1">هذا نقل داخلي بين خزانات فقط، وليس بيعًا أو توريدًا أو حركة مالية.</p><p className="font-medium text-amber-900">يرجى تأكيد تنفيذ النقل فقط إذا كنت متأكدًا من هذه الحركة.</p></div>}
      ...
      <div className="mt-5 flex justify-end gap-3">
        ...
        <button type="submit" className="ui-button" disabled={isSubmitting}>{isSubmitting ? 'جاري التنفيذ...' : hasDifferentFuelType ? 'Confirm transfer' : 'تأكيد النقل'}</button>
      </div>
    </form></div>}
  </main></PageLayout>;
}
