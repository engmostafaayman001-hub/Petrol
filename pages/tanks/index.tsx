  ...
  return <PageLayout title="الخزانات والمخزون"><main className="inventory-page">
    ...
    {transferOpen && <div className="modal-backdrop" onMouseDown={() => setTransferOpen(false)}><form onMouseDown={(event) => event.stopPropagation()} onSubmit={submitTransfer} className="ui-card form-card modal-card" style={{ maxWidth: 560 }}>
      ...
      <div className="space-y-4">
        ...
        {sourceTank && destinationTank && hasDifferentFuelType && <div className="rounded border border-amber-400 bg-amber-50 p-4 text-sm text-amber-950 shadow-sm"><p className="mb-2 text-base font-bold text-amber-900">تنبيه</p><p className="mb-2 font-semibold">نوع الوقود في الخزان المصدر مختلف عن نوع الوقود في الخزان المستهدف.</p><p className="mb-1">المصدر: <strong>{sourceTank.fuel_name || 'وقود'}</strong></p><p className="mb-1">المستهدف: <strong>{destinationTank.fuel_name || 'وقود'}</strong></p><p className="font-medium text-amber-900">هل تريد تأكيد عملية النقل؟</p></div>}
